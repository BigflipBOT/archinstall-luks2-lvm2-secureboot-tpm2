...
To build a working systemd based initramfs, modify the `HOOKS=` line in `/etc/mkinitcpio.conf`, by adding the following hooks: **systemd, keyboard, sd-vconsole, sd-encrypt, lvm2**. Your line should look simmilar to the following:
...
```
# pacman -Syu systemd-ukify sbsigntools efitools
```
...
# mkinitcpio -p linux
```
If everything went well, you should see `==> Unified kernel image generation successful`
...
```
If you prefer other means of managing network, refer to Arch Wiki [ network ](https://wiki.archlinux.org/title/Network_configuration) section.
